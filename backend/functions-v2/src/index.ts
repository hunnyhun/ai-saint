...
async function checkTaskScheduled(userId: string, type: 'notification_morning' | 'notification_evening', userToday: string): Promise<boolean> {
...
async function markTaskScheduled(userId: string, type: 'notification_morning' | 'notification_evening', userToday: string, scheduledTime: Date): Promise<void> {
    const scheduledMarkerRef = db.collection('users').doc(userId).collection('scheduledTasks').doc(`${userToday}_${type}`);
    try {
    ...
    try {
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            try {
                ...
                let targetSendType: 'notification_morning' | 'notification_evening' | null = null;
                let targetWindowStartHour: number | null = null;
                let targetWindowEndHour: number | null = null;
                    ...
                    const convos = await db.collection('users').doc(userId).collection('conversations')
                     ...
                     if (!convos.empty && convos.docs[0].data()?.messages) {
                         userMessages = convos.docs[0].data().messages
                             .filter((m: any) => m.role === 'user').map((m: any) => m.content).slice(0, 3);
                     }
                 } catch (histError) { console.error(`Error fetching history for quote gen for ${userId}`, histError); }

                 let quote = "May your day be blessed."; // Default
                 try {
                      quote = await generateSpiritualQuote(userMessages);
                 } catch(quoteError) {console.error(`Error generating quote for ${userId}`, quoteError); }
...
interface NotificationTaskPayload {
    userId: string;
    quote: string;
    sendType: 'notification_morning' | 'notification_evening';
...
export const sendNotificationTaskHandler = onRequest({
    ...
    try {
        // Decode payload
        let payload: NotificationTaskPayload;
        if (req.body.message && req.body.message.data) {
            ...
            payload = req.body as NotificationTaskPayload;
        ...
        for (let i = 0; i < deviceDocs.length; i += batchSize) {
            ...
            const messages: any[] = []; // Use any[] for flexibility with badge counts
            ...
            for (const deviceDoc of batchDocs) {
                 ...
                 try {
                     ...
                     } catch (readError) { console.error(`${logPrefix} Failed to read badge count for ${deviceToken.substring(0,10)}...`, readError); }
                    ...
                    const message = {
                        ...
                        android: { /* ... existing android config ... */
                             priority: 'normal' as const, // Use normal for background tasks
                             notification: { sound: 'default', channelId: 'daily_quotes', priority: 'default' as const, defaultSound: true, visibility: 'public' as const }
                        }
                    };
...
async function deleteQueryBatch(query: FirebaseFirestore.Query, resolve: () => void, reject: (error: any) => void) {
    try {
